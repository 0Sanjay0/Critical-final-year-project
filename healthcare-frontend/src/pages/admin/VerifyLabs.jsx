...
const UserCard = ({ user, profile, verificationDocs, onVerify }) => {
  ...
  const nameField = profile?.labName || profile?.labName || '—';

  return (
    <div className="card overflow-hidden">
      <div className="p-4 flex items-start gap-4">
        <div className="w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center font-semibold text-amber-700 flex-shrink-0 text-sm">
      ...
      {expanded && (
        <div className="border-t border-slate-100 p-4 bg-slate-50 space-y-4 animate-fade-in">
          {profile ? (
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  ['Type',            profile.labType || profile.labType],
            ...
            <p className="text-sm text-slate-400">Lab has not submitted their profile yet.</p>
          )}
...
const VerifyLabs = () => {
  ...
  const load = async () => {
    ...
    try {
      if (filter === 'pending' || filter === '') {
        ...
        let results = (res.data.data.results || []).filter(r => r.user.role === 'lab');
        if (filter === '') {
          const allRes = await adminService.getUsers({ role: 'lab' });
        ...
        const allRes = await adminService.getUsers({ role: 'lab', verificationStatus: filter });
  ...
  const verify = async (userId, status, rejectionReason) => {
    try { await adminService.verifyAccount(userId,{status,rejectionReason}); toast.success(`Lab ${status}.`); load(); }
  ...
  return (
    <div className="page-container animate-fade-in space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">Verify Labs</h1>
          <p className="text-slate-400 text-sm mt-1">Review lab applications and verification documents.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center"><p className="text-slate-400">No labs found.</p></div>
      ) : (
